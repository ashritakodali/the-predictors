...
yards <- c(10, 50)
weather_conds <- c('sunny', 'cloudy', 'snowing', 'foggy', 'raining',
                   'indoors', 'hot', 'cold', 'humid', 'windy')
df <- as.data.frame(cbind(yards, rep(weather_conds, each=2)))

for (i in 1:20){
  print(getyg(df[i,1], df[i,2]))
}
